...
export default function App() {
  ...
  // User không thuộc khoa vẫn có thể gọi nhưng không nhận cuộc gọi
  // Socket chỉ cần để gửi cuộc gọi, không cần đăng ký nhận cuộc gọi
  const identifier: RegisterData | null = useMemo(() => {
    if (!user) return null;
    // Vẫn tạo identifier để có socket gửi cuộc gọi
    // Nhưng không đăng ký nhận cuộc gọi (được xử lý trong IncomingCallWrapper)
    return {
      name: user.name || "",
  ...
  const handleConfirmCall = useCallback(async () => {
    try {
      // Nếu user không thuộc khoa, dùng tên user thay vì department_name
      ...
      if (res.data.success) {
        ...
        // Thêm call log cho mỗi đích gọi (outgoing call)
