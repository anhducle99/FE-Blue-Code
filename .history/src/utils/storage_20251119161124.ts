...
export const legacyStorage = {
  get: <T>(key: string): T | null => {
    try {
      ...
      try {
        ...
        // If parsing fails, it might be a plain string (like JWT token)
        // Return as string if T is string, otherwise return null
  ...
  set: <T>(key: string, value: T): boolean => {
    try {
      // If value is already a string, store it directly (for JWT tokens)
