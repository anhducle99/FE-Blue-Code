...
export default function App() {
  ...
  const selectedDepartmentNames = [
    ...departments.filter((d: Department) => selectedPhones.includes(d.phone)),
    ...supportContacts.filter((c: SupportContact) =>
      selectedPhones.includes(c.phone)
    ),
    ...
    <div className="min-h-screen h-screen overflow-y-auto bg-white">
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-grow px-4 pb-10">
          ...
          <div className="pt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-6 mt-8">
            {supportContacts.map((contact: SupportContact) => (
              <SupportButton
                key={contact.phone}
                label={contact.label}
                phone={contact.phone}
