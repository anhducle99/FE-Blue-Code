...
export const IncidentProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addIncident = useCallback(
    (incidentData: Omit<Incident, "id" | "timestamp">) => {
      ...
      // Always prepend new incidents to the beginning
  ...
  const loadCallHistory = useCallback(async () => {
    ...
    try {
      ...
      setIncidents((prevIncidents) => {
        // Get all existing incident IDs to avoid duplicates
        const existingIds = new Set(prevIncidents.map((i) => i.id));

        // If this is the initial load for this user, replace everything
        const isInitialLoad = !hasLoadedRef.current || loadedUserIdRef.current !== user.id;
        ...
        // For subsequent loads, merge intelligently
        // Keep ALL existing incidents (both system and real-time call logs)
        // Only add NEW call logs from database that don't already exist
        const newCallLogIncidents = convertedIncidents.filter(
          (i) => !existingIds.has(i.id)
        );

        // Merge: keep all existing + add new ones
