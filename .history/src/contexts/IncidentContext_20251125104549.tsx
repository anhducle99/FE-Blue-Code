...
export const IncidentProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const loadCallHistory = useCallback(async () => {
    ...
    try {
      ...
      setIncidents((prevIncidents) => {
        ...
        const isInitialLoad = !hasLoadedRef.current || isDifferentUser || prevIncidents.length === 0;
  ...
  useEffect(() => {
    if (user) {
      ...
      if (loadedUserIdRef.current !== (user.id || null)) {
        hasLoadedRef.current = false;
        loadedUserIdRef.current = null;
      }
      // Only load if not already loaded for this user
      if (!hasLoadedRef.current || loadedUserIdRef.current !== (user.id || null)) {
        loadCallHistory();
      }
    } else {
  ...
  }, [user?.id]); // Remove loadCallHistory from deps to prevent infinite loops
