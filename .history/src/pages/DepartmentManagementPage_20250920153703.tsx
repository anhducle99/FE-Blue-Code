...
export const DepartmentManagementPage: React.FC = () => {
  ...
  const allItems = [
    ...departments.map((d) => ({
      ...d,
      type: "department",
      alert_group: d.alert_group ?? "", // nếu Department có alert_group
    })),
    ...supportContacts.map((s) => ({
      ...
      phone: s.phone,
      type: "support",
      alert_group: "Lãnh Đạo", // thêm alert_group
    })),
  ];
              ...
              {allItems.map((d, index) => (
                <tr key={d.id} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    {(d as any).alert_group ||
                      (d.type === "support" ? "Lãnh Đạo" : "")}
                  </td>
                  <td className="px-4 py-2 text-right relative">
                    ...
                    {dropdownIndex === index && (
                      <div className="absolute right-0 mt-2 w-40 bg-white border rounded shadow-md z-50">
                        ...
                        <button
                          onClick={() => {
                            setEditingDept(d as IDepartment);
                            setIsOpen(true);
                            setDropdownIndex(null);
                          }}
                          className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100"
