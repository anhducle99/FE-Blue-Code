...
import { Department } from "../contexts/DashboardProvider";
...
export const DepartmentManagementPage: React.FC<Props> = ({
  departments,
  setDepartments,
}) => {
  ...
  const handleSave = () => {
    ...
    if (editingDept && editingDept.id) {
      // Cập nhật khoa/phòng
      const updatedDept = { ...editingDept, ...formData };
      setDepartments(
        departments.map((d) => (d.id === editingDept.id ? updatedDept : d))
      );
