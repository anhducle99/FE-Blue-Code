  ...
  const formik = useFormik<IUserForm & { departmentName: string }>({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Role chỉ dùng để hiển thị, không gửi lên backend
        const role = values.departmentName.toLowerCase().includes("lãnh đạo")
          ? "Admin"
          : "User";

        // Payload gửi đúng chuẩn backend
        const payloadCreate: IUserForm = {
          name: values.name,
          email: values.email,
          ...
          is_department_account: values.is_department_account,
          is_admin_view: values.is_admin_view,
        };

        const payloadUpdate: IUserForm & { password?: string } = {
          name: values.name,
          email: values.email,
          phone: values.phone,
          organization_id: values.organization_id,
        ...
        if (values.password) payloadUpdate.password = values.password;

        if (editingUser) {
          const updated = await updateUser(editingUser.id, payloadUpdate);
          setUsers((prev) =>
            prev.map((u) => (u.id === updated.id ? updated : u))
          );
          message.success("Cập nhật người dùng thành công");
        } else {
          const created = await createUser(payloadCreate);
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Select
                ...
                onChange={(val) => formik.setFieldValue("organization_id", val)}
                className="w-full"
              >
