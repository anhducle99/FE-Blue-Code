...
export const DepartmentManagementPage: React.FC = () => {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      const leaderGroup: LeaderContact[] = allDepts
  ...
  const handleDelete = async (id?: number) => {
    if (!id || !confirm("Bạn có chắc muốn xóa khoa/phòng này?")) return;
    try {
  ...
  return (
    <>
      ...
      <Modal
        ...
        onOk={async () => {
          // Validate bắt buộc
          if (!formData.name?.trim() || !formData.alert_group?.trim()) {
            message.error("Tên và nhóm báo động là bắt buộc");
            return;
          }
          ...
          try {
            if (editingDept && editingDept.id) {
              // Update
              const res = await updateDepartment(editingDept.id, payload);
              if (!res.data.success) throw new Error("Update thất bại");
            } else {
              // Create
              const res = await createDepartment(payload);
              if (!res.data.success) throw new Error("Tạo mới thất bại");
            }
            ...
            setEditingDept(null);
            setFormData({ name: "", phone: "", alert_group: "" });
            fetchDepartments(); // reload dữ liệu
          } catch (err) {
            console.error(err);
            message.error("Lưu thất bại");
          }
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm mb-1">Nhóm báo động *</label>
            <Select
