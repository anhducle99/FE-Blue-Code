...
export const StatisticsPage: React.FC = () => {
  ...
  const normalizeGroup = (group?: string) =>
    group?.trim().toLowerCase() || "default";
  ...
  const groupChartData = (label: "Gửi thông báo" | "Nhận thông báo") => {
    const isSent = label === "Gửi thông báo";
    ...
    return {
      ...
      datasets: [
        {
          label,
          data: groups.map((g) => (isSent ? g.sent : g.received)),
          backgroundColor: groups.map(
            (g) => alertGroupColors[normalizeGroup(g.alert_group)] || "#999"
            groups.forEach((g) => {
  console.log(g.alert_group, "=>", normalizeGroup(g.alert_group), "=>", alertGroupColors[normalizeGroup(g.alert_group)]);
});
          ),
          borderWidth: 1,
        },
