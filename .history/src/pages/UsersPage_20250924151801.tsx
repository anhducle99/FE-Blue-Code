...
export const UsersPage: React.FC = () => {
  ...
  // find với kiểu rõ ràng
  const organizationName =
    organizations.find((o: Organization) => o.id === values.organizationId)
      ?.name || "";

  const organizationId =
    organizations.find((o: Organization) => o.name === user.organization)?.id ||
    0;

  // map với kiểu rõ ràng
  {
  ...
  const formik = useFormik<IUserForm & { organizationId: number }>({
    initialValues: {
      organizationId: 0,
      department: "",
      name: "",
    ...
    onSubmit: async (values) => {
      try {
        const organizationName =
          organizations.find((o) => o.id === values.organizationId)?.name || "";
  ...
  const handleOpenEdit = (user: IUser) => {
    ...
    formik.setValues({
      organizationId:
        organizations.find((o) => o.name === user.organization)?.id || 0,
            ...
            {users.length > 0 ? (
              users.map((d, index) => (
                <tr key={d.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">{d.organization}</td>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <Select
                ...
                value={formik.values.organizationId || undefined}
                onChange={(val) => formik.setFieldValue("organizationId", val)}
                onBlur={() => formik.setFieldTouched("organizationId", true)}
                className="w-full"
              >
