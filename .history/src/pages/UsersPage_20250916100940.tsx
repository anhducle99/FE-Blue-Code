...
export const UsersPage: React.FC = () => {
  ...
  const handleOpenEdit = (user: User) => {
    setIsOpen(true); // mở modal trước
    setEditingUser(user); // set editingUser
    formik.setValues({
      ...
      role: user.role || "User", // nhớ thêm role nếu dùng IUserForm
    });
  };
                      ...
                      <div className="absolute right-0 mt-2 w-44 bg-white border rounded shadow-md z-50">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (
                              !confirm(
                                `Bạn có chắc muốn cấp lại mật khẩu cho ${d.name}?`
                              )
                            )
                              return;
                            try {
                              // Gọi API reset password, ví dụ: updateUser(id, { password: "123456Ab@" })
                              await updateUser(d.id, { password: "123456Ab@" });
                              message.success(
                                `Cấp lại mật khẩu cho ${d.name} thành công`
                              );
                            } catch (err) {
                              console.error(err);
