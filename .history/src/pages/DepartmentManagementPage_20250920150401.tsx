...
export const DepartmentManagementPage: React.FC = () => {
  ...
  // Mapping nhóm alert_group sang màu cho supportContacts
  ...
  const fetchDepartments = async () => {
    try {
      ...
      // Phân nhóm tự động
      const deptGroup = allDepts
        .filter((d) => d.alert_group === "Y tế")
        .map((d) => ({
          name: d.name!,
          phone: d.phone || "", // đảm bảo luôn là string
        }));

      const supportGroup = allDepts
        .filter(
          (d) =>
            d.alert_group &&
            d.alert_group !== "Y tế" &&
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Cập nhật context lại sau khi thao tác
      fetchDepartments();
    } catch (err) {
      console.error(err);
  ...
  // Kết hợp departments + supportContacts để render table
  const allItems = [
    ...departments.map((d) => ({
      name: d.name,
      phone: d.phone,
