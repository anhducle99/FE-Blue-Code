...
export const StatisticsPage: React.FC = () => {
  ...
  const groupChartData = (
    groups: IGroupStats[],
    label: "Gửi thông báo" | "Nhận thông báo"
  ) => {
    // Log để debug alert_group và màu
    console.log(
      groups.map((g) => ({
        label: g.label,
        alert_group: g.alert_group,
        color: alertGroupColors[g.alert_group?.trim() || ""] ?? "#999",
      }))
    );
      ...
      datasets: [
        {
          ...
          backgroundColor: groups.map(
            (g) => alertGroupColors[g.alert_group?.trim() || ""] ?? "#999"
          ),
          borderWidth: 1,
        },
      ],
