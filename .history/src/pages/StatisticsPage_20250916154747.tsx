...
export const StatisticsPage: React.FC = () => {
  ...
  const fetchDepartmentStats = async () => {
    ...
    try {
      ...
      // Convert string → number nếu BE trả string
      ...
      console.log("📊 Dept API response:", depts);
      setDepartments(depts);
    } catch (err) {
      console.error("❌ Lỗi fetchDepartmentStats:", err);
    }
  };
  ...
  const fetchGroupStats = async () => {
    ...
    try {
      ...
      const grp = res.data.data.map((g) => ({
        label: g.label,
        sent: Number(g.sent),
        received: Number(g.received),
      }));
      console.log("📊 Group API response:", grp);
      setGroups(grp);
    } catch (err) {
      console.error("❌ Lỗi fetchGroupStats:", err);
    }
  };
