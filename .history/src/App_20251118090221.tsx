...
export default function App() {
  ...
  const [incidentSidebarOpen, setIncidentSidebarOpen] = useState(true);
  ...
  const makeKey = useCallback((name: string, dept: string) => `${name}_${dept}`, []);
  ...
  }, [departments, supportContacts, currentUser.department_name, currentUser.name, makeKey]);
  ...
  return (
    <div className="min-h-screen h-screen overflow-hidden bg-white flex">
      <div className={`flex flex-col min-h-screen flex-1 transition-all duration-300 ${incidentSidebarOpen ? 'mr-[33.333%]' : ''}`}>
        <div className="flex-shrink-0">
          <Header />
          <div className="flex items-center justify-end px-4 py-2 bg-gray-50 border-b">
            <button
              onClick={() => setIncidentSidebarOpen(!incidentSidebarOpen)}
              className="bg-red-600 text-white rounded px-3 py-1 text-sm hover:bg-red-700 flex items-center gap-2"
              title="Sự cố và cảnh báo"
            >
      ...
      <IncidentSidebar
        isOpen={incidentSidebarOpen}
        onClose={() => setIncidentSidebarOpen(false)}
      />
    </div>
  );
}
