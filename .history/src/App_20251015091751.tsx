...
export default function App() {
  ...
  const identifier: RegisterData = useMemo(
    () => ({
      name: currentUser.name,
      department_id: String(currentUser.department_id),
      department_name: currentUser.department_name,
    }),
    [currentUser.name, currentUser.department_id, currentUser.department_name]
  );
  ...
  const handleConfirmCall = async () => {
    try {
      const res = await axios.post("http://localhost:5000/api/call", {
        ...
        message: tempMessage,
        targetKeys: selectedKeys, // ✅ gửi theo key chuẩn
      });
  ...
  // --- Gọi trực tiếp qua input key (logic HomePage) ---
    ...
    <div className="min-h-screen h-screen overflow-y-auto bg-white">
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-grow px-4 pb-10">
          {/* 🔹 Input gọi nhanh (HomePage logic) */}
          <div className="flex gap-2 mb-6 mt-6">
            <input
              className="border px-3 py-2 rounded w-96"
              placeholder="Nhập key của khoa cần gọi (vd: khoabbb_YHCT - Cơ xương khớp)"
              value={targetKey}
              onChange={(e) => setTargetKey(e.target.value)}
            />
            <button
              className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
              onClick={makeCallDirect}
            >
              Gọi nhanh
            </button>
          </div>
