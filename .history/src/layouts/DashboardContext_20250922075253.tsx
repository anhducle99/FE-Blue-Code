...
export const DashboardProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get("/api/departments"); // API backend
      const allItems = Array.isArray(res.data) ? res.data : [];

      const deptGroup: Department[] = allItems
      ...
      sessionStorage.setItem("departments", JSON.stringify(deptGroup));

      const supportGroup: SupportContact[] = allItems
        .filter((d) =>
      ...
      sessionStorage.setItem("supportContacts", JSON.stringify(supportGroup));

      const leaderGroup: LeaderContact[] = allItems
      ...
      sessionStorage.setItem("leaders", JSON.stringify(leaderGroup));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const savedDepartments = sessionStorage.getItem("departments");
    const savedSupport = sessionStorage.getItem("supportContacts");
    const savedLeaders = sessionStorage.getItem("leaders");

    if (savedDepartments) setDepartments(JSON.parse(savedDepartments));
    if (savedSupport) setSupportContacts(JSON.parse(savedSupport));
