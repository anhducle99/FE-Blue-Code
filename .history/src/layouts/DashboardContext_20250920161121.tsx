...
export interface SupportContact {
  ...
  phone: string;
  color: string;
  alert_group: string; // PHẢI ĐÚNG TÊN
}
...
export const DashboardProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get("/api/departments"); // API backend của bạn
      ...
      setSupportContacts(
        allItems
          ...
          .map((d) => ({
            ...
            alert_group: d.alert_group, // phải đúng tên
          }))
      );
