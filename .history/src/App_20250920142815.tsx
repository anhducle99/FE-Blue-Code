...
import {
  DashboardProvider,
  useDashboardContext,
} from "./contexts/DashboardProvider"; // import đúng
...
// Tách component bên trong để dùng hook context
const DashboardAppContent: React.FC = () => {
  const location = useLocation();
  const isLoginPage = location.pathname === "/login";

  const { group1, group2 } = useDashboardContext(); // lấy group1, group2 từ context
  const departments = [...group1, ...group2]; // gộp để hiển thị
  const supportContacts = []; // nếu có supportContacts bạn cũng có thể đưa vào context
...
// Bọc App bằng DashboardProvider
