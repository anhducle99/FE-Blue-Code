...
// Component con để dùng hook useDashboardContext
const DashboardAppContent: React.FC = () => {
  const location = useLocation();
  const isLoginPage = location.pathname === "/login";

  const { group1, group2 } = useDashboardContext();
  const departments = [...group1, ...group2]; // gộp 2 nhóm
  const supportContacts: any[] = []; // nếu có, bạn có thể thêm supportContacts vào context
