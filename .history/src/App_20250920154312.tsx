...
export default function App() {
  ...
  // Chỉ lấy departments và supportContacts đã lọc “Lãnh Đạo”
  const visibleDepartments = departments.filter(
    (d) => (d as any).alert_group !== "Lãnh Đạo"
  );

  const visibleSupportContacts = supportContacts; // supportContacts đã là Lãnh Đạo nếu muốn hiển thị riêng
  ...
  const selectAll = () => {
    setSelectedPhones([
      ...visibleDepartments.map((d) => d.phone),
      ...visibleSupportContacts.map((s) => s.phone),
    ]);
  };
  ...
  const selectedDepartmentNames = [
    ...visibleDepartments.filter((d) => selectedPhones.includes(d.phone)),
    ...visibleSupportContacts.filter((c) => selectedPhones.includes(c.phone)),
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-grow px-4 pb-10">
          ...
          <div className="pt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-6 mt-8">
            {visibleDepartments.map((dept: Department) => (
              <DepartmentButton
                key={dept.id}
                name={dept.name}
                phone={dept.phone}
          ...
          {/* SupportContacts */}
          <div className="pt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-6 mt-8">
            {visibleSupportContacts.map((contact: SupportContact) => (
              <SupportButton
                key={contact.id}
                label={contact.label}
                phone={contact.phone}
