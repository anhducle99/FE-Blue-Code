...
export default function App() {
  ...
  const selectAll = () => {
    const allowedDeptKeys = departments
      .filter((d) => d.name !== currentUser.department_name) // ❌ bỏ khoa của chính mình
      .map((d) => makeKey(d.name, d.name));

    const allowedSupportKeys = supportContacts
      .filter(
        (s) =>
          s.label !== currentUser.department_name && // ❌ bỏ nếu trùng khoa
          s.label !== currentUser.name // ❌ bỏ nếu trùng tên user
      )
