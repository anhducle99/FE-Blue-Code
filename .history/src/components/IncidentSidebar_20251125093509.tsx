...
const IncidentSidebar: React.FC<IncidentSidebarProps> = ({
  ...
  // Get color for call log based on call type or status
  const getCallLogColor = (incident: Incident): string => {
    // Priority 1: Check callType (explicit call log type)
    if ((incident as any).callType) {
      ...
      if (callType === "accepted") {
        return "#22c55e"; // Green for accepted calls
      }
      if (callType === "rejected" || callType === "timeout") {
        return "#facc15"; // Yellow for rejected/timeout calls
      }
    }

    // Priority 2: Auto-detect from message content (for call logs)
    ...
    // Priority 3: Fallback to status-based colors for non-call incidents
