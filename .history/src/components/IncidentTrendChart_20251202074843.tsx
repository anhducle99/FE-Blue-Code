...
const IncidentTrendChart = () => {
  ...
  const { labels, totals, unresolved, totalCount, unresolvedCount } = useMemo(() => {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Theo dõi số lượng sự cố trong {TIMEFRAMES[timeframe].label.toLowerCase()}
          </p>
        </div>
          ...
          {(Object.keys(TIMEFRAMES) as Timeframe[]).map((key) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-sm font-medium transition ${
                timeframe === key ? "bg-blue-500 text-white" : "bg-slate-800 text-slate-300"
              }`}
            >
