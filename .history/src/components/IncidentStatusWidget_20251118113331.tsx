...
const IncidentStatusWidget: React.FC = () => {
  ...
  // Hàm kiểm tra xem incident có thuộc ngày hiện tại không
  const isToday = (incident: Incident): boolean => {
    const incidentDate = incident.timestamp instanceof Date 
      ? incident.timestamp 
  ...
  const stats = useMemo(() => {
    // Lọc chỉ lấy incidents trong ngày hiện tại
    ...
    const warnings = todayIncidents.filter((i) => i.status === "warning").length;
  ...
  const gaugePercentage =
    todayIncidents.length > 0 ? (stats.online / todayIncidents.length) * 100 : 100;
