...
const IncidentStatusWidget: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const resolvedPercentage = total > 0 ? ((resolved / total) * 100).toFixed(1) : "0.0";
    const totalPercentage = total > 0 ? ((online / total) * 100).toFixed(1) : "100.0";
    ...
    const statusColor = errors === 0 ? "bg-green-500" : errors <= 2 ? "bg-yellow-500" : "bg-red-500";
  ...
  const gaugePercentage = incidents.length > 0 
    ? ((stats.online / incidents.length) * 100)
    : 100;
  ...
  return (
    <div className="h-full flex flex-col bg-slate-800 p-4">
      ...
      <div 
        ...
        style={{
          backgroundColor: stats.status === "OK" ? "#22c55e" : stats.status === "WARNING" ? "#facc15" : "#ef4444"
        }}
      >
